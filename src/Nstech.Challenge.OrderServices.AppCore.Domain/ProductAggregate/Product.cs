...
public class Product : Entity
{
    ...
    public static ValueResult<Product> Create(Guid id, string sku, decimal unitPrice, int availableQuantity)
    {
        ...
        if (failures.Any())
        {
            return ValueResult<Product>.Failure(failures);
        }

        return ValueResult<Product>.Success(new Product(id, sku, unitPrice, availableQuantity));
    }
    ...
    public ValueResult<Product> ReserveStock(int quantity)
    {
        if (quantity <= 0)
        {
            return ValueResult<Product>.Failure("Quantity must be greater than 0", "Quantity", "INVALID_QUANTITY");
        }

        if (!HasSufficientStock(quantity))
        {
            return ValueResult<Product>.Failure($"Insufficient stock. Available: {AvailableQuantity}, Requested: {quantity}", "Stock", "INSUFFICIENT_STOCK");
        }

        AvailableQuantity -= quantity;
        ReservedQuantity += quantity;

        return ValueResult<Product>.Success(this);
    }

    public ValueResult<Product> ReleaseReservedStock(int quantity)
    {
        if (quantity <= 0)
        {
            return ValueResult<Product>.Failure("Quantity must be greater than 0", "Quantity", "INVALID_QUANTITY");
        ...
        {
            return ValueResult<Product>.Failure($"Cannot release more than reserved. Reserved: {ReservedQuantity}, Release: {quantity}", "Stock", "INVALID_RELEASE");
        }
